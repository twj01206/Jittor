...
class Discriminator(nn.Module):
    ...
    def execute(self, img, labels):
        d_in = jt.contrib.concat(
            (img.view((img.shape[0], (-1))), self.label_embedding(labels)), dim=1
        )
        # TODO: 将d_in输入到模型中并返回计算结果
        d_out = self.model(d_in)  # TOOP:
        return d_out
...
def sample_image(n_row, batches_done):
    # 随机采样输入并保存生成的图片
    z = (
        jt.array(np.random.normal(0, 1, (n_row**2, opt.latent_dim)))
        .float32()
        .stop_grad()
    )
    labels = (
        jt.array(np.array([num for _ in range(n_row) for num in range(n_row)]))
        .float32()
        .stop_grad()
    )
...
number = "13928533570"  # TODO: 写入比赛页面中指定的数字序列（字符串类型）
n_row = len(number)
z = jt.array(np.random.normal(0, 1, (n_row, opt.latent_dim))).float32().stop_grad()
labels = (
    jt.array(np.array([int(number[num]) for num in range(n_row)])).float32().stop_grad()
)
gen_imgs = generator(z, labels)
